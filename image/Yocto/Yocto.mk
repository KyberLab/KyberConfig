...
#IMAGE_FETCH_METHOD			:= 
IMAGE_FETCH_OPTS			:= 
IMAGE_FETCH_URL				:= git://git.yoctoproject.org/poky
...
IMAGE_INSTALL_OPTS			:= 
IMAGE_INSTALL_LIST			:= build/$(IMAGE_YOCTO_FILE_NAME):$(IMAGE_YOCTO_FILE_NAME) build/tmp/deploy/images/qemuriscv64/qemuriscv64.bin:Yocto-Image.bin
IMAGE_INSTALL_LIST			+= build/tmp/deploy/images/qemuriscv64/Image-initramfs-qemuriscv64.bin:Yocto-Initramfs.bin

# (Optional) Package options
#IMAGE_PACKAGE_METHOD		:= 
IMAGE_PACKAGE_OPTS			:= build/$(IMAGE_YOCTO_FILE_NAME):$(IMAGE_YOCTO_FILE_NAME) build/tmp/deploy/images/qemuriscv64/qemuriscv64.bin:Yocto-Image.bin
IMAGE_PACKAGE_LIST			:= 
...
define image_postrun_fetch
	$(IQ)cd $(3) && \
		git clone git://git.openembedded.org/meta-openembedded.git -b $(YOCTO_BUILD_BRANCH) && \
		git clone git://git.yoctoproject.org/meta-riscv.git -b $(YOCTO_BUILD_BRANCH) && \
		git clone git://git.yoctoproject.org/meta-virtualization.git -b $(YOCTO_BUILD_BRANCH) || true
endef
